...
| **TBSS-FR-0008** | [Record tab — recordings browser (full directory listing, waveforms, region export) + repeat-take fix](TBSS-FR-0008-record-tab-recordings-browser.md) | 📝 Proposed | — | List every WAV in the recordings filespace (not just manifested), path-label copy/open-in-Explorer buttons, fix the "Record won't start again after a take is listed" bug, per-take waveform thumbnail + click-drag region selection + Export Selection (lossless crop via `trim::crop_wav_bytes`). |
