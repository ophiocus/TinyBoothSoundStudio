...
fn render_body(app: &mut TinyBoothApp, ui: &mut egui::Ui) {
    ui.horizontal(|ui| {
        if ui.button("+ New").on_hover_text("Duplicate the selected profile").clicked() {
            let src = app
                .admin_edit_idx
...
fn render_editor(p: &mut Profile, ui: &mut egui::Ui) {
    egui::ScrollArea::vertical().show(ui, |ui| {
        ...
        row(ui, "Input gain (dB)", |ui| {
            ui.add(egui::DragValue::new(&mut p.input_gain_db).speed(0.1).suffix(" dB").range(-24.0..=24.0));
        });

        ui.add_space(10.0);
        ui.strong("High-pass filter");
        row(ui, "Enabled", |ui| { ui.checkbox(&mut p.hpf_enabled, ""); });
        row(ui, "Cutoff (Hz)", |ui| {
            ui.add(egui::DragValue::new(&mut p.hpf_hz).speed(1.0).suffix(" Hz").range(20.0..=1000.0));
        });

        ui.add_space(10.0);
        ui.strong("Noise gate");
        row(ui, "Enabled", |ui| { ui.checkbox(&mut p.gate_enabled, ""); });
        row(ui, "Threshold (dB)", |ui| {
            ui.add(egui::DragValue::new(&mut p.gate_threshold_db).speed(0.5).suffix(" dB").range(-80.0..=0.0));
        });
        row(ui, "Attack (ms)", |ui| {
            ui.add(egui::DragValue::new(&mut p.gate_attack_ms).speed(0.5).suffix(" ms").range(0.1..=200.0));
        });
        ...
        row(ui, "Enabled", |ui| { ui.checkbox(&mut p.compressor_enabled, ""); });
        row(ui, "Threshold (dB)", |ui| {
            ui.add(egui::DragValue::new(&mut p.compressor_threshold_db).speed(0.5).suffix(" dB").range(-60.0..=0.0));
        });
        row(ui, "Ratio (x:1)", |ui| {
            ui.add(egui::DragValue::new(&mut p.compressor_ratio).speed(0.1).range(1.0..=20.0));
        });
        row(ui, "Attack (ms)", |ui| {
            ui.add(egui::DragValue::new(&mut p.compressor_attack_ms).speed(0.5).suffix(" ms").range(0.1..=200.0));
        });
        row(ui, "Release (ms)", |ui| {
            ui.add(egui::DragValue::new(&mut p.compressor_release_ms).speed(1.0).suffix(" ms").range(1.0..=2000.0));
        });
        row(ui, "Makeup gain (dB)", |ui| {
            ui.add(egui::DragValue::new(&mut p.compressor_makeup_db).speed(0.1).suffix(" dB").range(-12.0..=24.0));
        });
