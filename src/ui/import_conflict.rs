...
pub fn show(app: &mut TinyBoothApp, ctx: &egui::Context) {
    let Some(pending) = app.import_conflict.as_ref() else { return };
    ...
    let existing_name = probe.existing_project_name.clone().unwrap_or_else(|| "(unknown)".into());
    ...
    egui::Window::new("⚠  Suno session already imported")
        ...
        .show(ctx, |ui| {
            ...
            egui::Grid::new("conflict_grid").num_columns(2).spacing([12.0, 4.0]).show(ui, |ui| {
            ...
            ui.add_space(10.0);
            ui.horizontal(|ui| {
                if ui.add(egui::Button::new("Replace existing project").min_size(egui::vec2(200.0, 30.0))).clicked() {
                    click_replace = true;
                }
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    if ui.add(egui::Button::new("Cancel").min_size(egui::vec2(120.0, 30.0))).clicked() {
                        click_cancel = true;
                    }
                });
            });
    ...
    if click_replace { app.resolve_import_conflict(true); }
    if click_cancel  { app.resolve_import_conflict(false); }
}
